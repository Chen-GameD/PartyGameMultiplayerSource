...
TArray<FString> UReturnGameState::GetTeam1PlayerData(int32 index)
{
	...
	return TArray<FString>({keyList[index], FString::FromInt(playerDataT1[keyList[index]][0]), FString::FromInt(playerDataT1[keyList[index]][1]), FString::FromInt(playerDataT1[keyList[index]][2])});
}
...
TArray<FString> UReturnGameState::GetTeam2PlayerData(int32 index)
{
	...
	return TArray<FString>({keyList[index], FString::FromInt(playerDataT2[keyList[index]][0]), FString::FromInt(playerDataT2[keyList[index]][1]), FString::FromInt(playerDataT2[keyList[index]][2])});
}

void UReturnGameState::AddPlayerData(bool isTeam1, FString username, int32 kills, int32 deaths, int32 score)
{
	if(isTeam1)
	{
		...
		{
			playerDataT1.Add(username, TArray<int>({kills, deaths, score}));
		}
	}
	else
	{
		...
		{
			playerDataT2.Add(username, TArray<int>({kills, deaths, score}));
		}
	}
}
